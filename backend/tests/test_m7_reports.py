...
def test_dashboard_summary_matches_current_database_formula_and_persists_snapshot(auth_headers) -> None:
    ...
    deposit_receipt_doc = _query_receipt_doc(contract_id=sales_contract_id, doc_type="DEPOSIT")
    normal_receipt_doc = _query_receipt_doc(contract_id=sales_contract_id, sales_order_id=sales_order_id, doc_type="NORMAL")
    deposit_payment_doc = _query_payment_doc(contract_id=purchase_contract_id, doc_type="DEPOSIT")
    normal_payment_doc = _query_payment_doc(contract_id=purchase_contract_id, purchase_order_sales_order_id=sales_order_id, doc_type="NORMAL")
    ...
    assert Decimal(str(body["contract_execution_rate"])) == expected["contract_execution_rate"]
    assert Decimal(str(body["actual_receipt_today"])) == expected["actual_receipt_today"]
    assert Decimal(str(body["actual_payment_today"])) == expected["actual_payment_today"]
    assert Decimal(str(body["inventory_turnover_30d"])) == expected["inventory_turnover_30d"]
    ...
    assert Decimal(str(snapshot.metric_payload["contract_execution_rate"])) == expected["contract_execution_rate"]


def test_board_tasks_returns_counts_and_contains_current_actionable_items(auth_headers) -> None:
    pending_sales_contract_id = _create_effective_sales_contract(auth_headers, qty_signed=Decimal("100.000"))
    pending_purchase_contract_id = _create_effective_purchase_contract(auth_headers, qty_signed=Decimal("100.000"))
    ...
    validation_purchase_contract_id = _create_effective_purchase_contract(auth_headers, qty_signed=Decimal("100.000"))
    validation_inbound_doc = _query_inbound_doc(contract_id=validation_purchase_contract_id)
    ...
    qty_done_sales_contract_id = _create_effective_sales_contract(auth_headers, qty_signed=Decimal("100.000"))
    qty_done_purchase_contract_id = _create_effective_purchase_contract(auth_headers, qty_signed=Decimal("100.000"))
    ...
    qty_done_deposit_doc = _query_receipt_doc(contract_id=qty_done_sales_contract_id, doc_type="DEPOSIT")
    ...
    assert any(item["biz_id"] == pending_receipt_doc.id for item in body["pending_supplement_items"])
    assert any(item["biz_id"] == validation_inbound_doc.id for item in body["validation_failed_items"])
    assert any(item["biz_id"] == qty_done_sales_contract_id for item in body["qty_done_not_closed_items"])
...
def test_light_overview_blocks_non_operator_roles_and_matches_current_formula(auth_headers) -> None:
...
def _compute_expected_dashboard_metrics() -> dict[str, Decimal | int]:
    ...
    try:
        ...
        for contract in contracts:
            if contract.status not in {"生效中", "数量履约完成", "已关闭", "手工关闭", "已归档"}:
                continue
            ...
            for item in contract_with_items.items:
                ...
                done_qty = Decimal(str(item.qty_in_acc if contract.direction == "purchase" else item.qty_out_acc))
                total_signed += signed_qty
                total_done += min(done_qty, signed_qty)
        contract_execution_rate = Decimal("0.000000") if total_signed <= Decimal("0.000") else (total_done / total_signed).quantize(Decimal("0.000001"))

        start_utc, end_utc = _today_window_utc()
        actual_receipt_today = Decimal("0.00")
        for doc in db.scalars(select(ReceiptDoc).where(ReceiptDoc.status.in_(["已确认", "已核销"]))).all():
            if doc.confirmed_at and start_utc <= doc.confirmed_at.astimezone(UTC) < end_utc:
                actual_receipt_today += Decimal(str(doc.amount_actual)) - Decimal(str(doc.refund_amount))
        actual_receipt_today = actual_receipt_today.quantize(Decimal("0.01"))

        actual_payment_today = Decimal("0.00")
        for doc in db.scalars(select(PaymentDoc).where(PaymentDoc.status.in_(["已确认", "已核销"]))).all():
            if doc.confirmed_at and start_utc <= doc.confirmed_at.astimezone(UTC) < end_utc:
                actual_payment_today += Decimal(str(doc.amount_actual)) - Decimal(str(doc.refund_amount))
        ...
        for doc in db.scalars(select(InboundDoc).where(InboundDoc.status == "已过账")).all():
            current_available_stock += Decimal(str(doc.actual_qty))
        for doc in db.scalars(select(OutboundDoc).where(OutboundDoc.status == "已过账")).all():
            qty = Decimal(str(doc.actual_qty))
            current_available_stock -= qty
            if doc.submitted_at and recent_start <= doc.submitted_at.astimezone(UTC) < recent_end:
                recent_outbound += qty
        inventory_turnover_30d = Decimal("0.000000")
        if current_available_stock > Decimal("0.000"):
            inventory_turnover_30d = (recent_outbound / current_available_stock).quantize(Decimal("0.000001"))
...
def _compute_expected_board_metrics() -> dict[str, int]:
    ...
    try:
        pending_supplement_count = len(
            db.scalars(select(ReceiptDoc).where(ReceiptDoc.status == "待补录金额")).all()
        ) + len(db.scalars(select(PaymentDoc).where(PaymentDoc.status == "待补录金额")).all())
        validation_failed_count = len(
            db.scalars(select(InboundDoc).where(InboundDoc.status == "校验失败")).all()
        ) + len(db.scalars(select(OutboundDoc).where(OutboundDoc.status == "校验失败")).all())
        ...
        return {
            ...
            "total_alert_count": pending_supplement_count + validation_failed_count + qty_done_not_closed_count,
        }
    finally:
        db.close()
...
def _compute_expected_light_metrics() -> dict[str, Decimal | int]:
    ...
    try:
        ...
        for doc in db.scalars(select(InboundDoc).where(InboundDoc.status == "已过账")).all():
            if doc.submitted_at and start_utc <= doc.submitted_at.astimezone(UTC) < end_utc:
...
def _create_effective_sales_contract(
    ...
    *,
    ...
    create_response = client.post(
        ...
        headers=_finance_headers(auth_headers, f"CODEX-TEST-M7-FINANCE-SC-{uuid4().hex[:6]}"),
    )
    ...
    submit_response = client.post(
        ...
        headers=_finance_headers(auth_headers, f"CODEX-TEST-M7-FINANCE-SC-SUB-{uuid4().hex[:6]}"),
    )
    assert submit_response.status_code == 200
    approve_response = client.post(
        f"/api/v1/contracts/{contract_id}/approve",
        json={"approval_result": True, "comment": "M7 销售合同生效"},
        headers=_finance_headers(auth_headers, f"CODEX-TEST-M7-FINANCE-SC-APP-{uuid4().hex[:6]}"),
    )
...
def _create_effective_purchase_contract(
    ...
    *,
    ...
    create_response = client.post(
        ...
        headers=_finance_headers(auth_headers, f"CODEX-TEST-M7-FINANCE-PC-{uuid4().hex[:6]}"),
    )
    assert create_response.status_code == 200
    contract_id = create_response.json()["id"]
    submit_response = client.post(
        ...
        headers=_finance_headers(auth_headers, f"CODEX-TEST-M7-FINANCE-PC-SUB-{uuid4().hex[:6]}"),
    )
    assert submit_response.status_code == 200
    approve_response = client.post(
        f"/api/v1/contracts/{contract_id}/approve",
        json={"approval_result": True, "comment": "M7 采购合同生效"},
        headers=_finance_headers(auth_headers, f"CODEX-TEST-M7-FINANCE-PC-APP-{uuid4().hex[:6]}"),
    )
    assert approve_response.status_code == 200
    return contract_id
...
def _create_sales_order_derived(
    ...
    *,
    ...
    ops_response = client.post(
        ...
        headers=_ops_admin_web_headers(auth_headers, f"CODEX-TEST-M7-OPS-{uuid4().hex[:6]}"),
    )
    ...
    finance_response = client.post(
        ...
        headers=_finance_headers(auth_headers, f"CODEX-TEST-M7-FINANCE-ORDER-{uuid4().hex[:6]}"),
    )
...
def _confirm_receipt_doc(auth_headers, receipt_doc_id: int, amount_actual: Decimal) -> None:
    response = client.post(
        ...
        headers=_finance_headers(auth_headers, f"CODEX-TEST-M7-FINANCE-RECEIPT-{uuid4().hex[:6]}"),
    )
    assert response.status_code == 200
...
def _confirm_payment_doc(auth_headers, payment_doc_id: int, amount_actual: Decimal) -> None:
    response = client.post(
        ...
        headers=_finance_headers(auth_headers, f"CODEX-TEST-M7-FINANCE-PAY-{uuid4().hex[:6]}"),
    )
    assert response.status_code == 200
...
def _create_system_outbound_doc(
    ...
    *,
    ...
    response = client.post(
        ...
        headers=_warehouse_headers(auth_headers, f"CODEX-TEST-M7-WH-CREATE-{uuid4().hex[:6]}"),
    )
    assert response.status_code == 200
    return response.json()["id"]


def _submit_outbound_doc(auth_headers, outbound_doc_id: int, actual_qty: Decimal) -> None:
    response = client.post(
        f"/api/v1/outbound-docs/{outbound_doc_id}/submit",
        json={"actual_qty": float(actual_qty), "warehouse_id": "CODEX-TEST-WH-M7-001"},
        headers=_warehouse_headers(auth_headers, f"CODEX-TEST-M7-WH-SUB-OUT-{uuid4().hex[:6]}"),
    )
    assert response.status_code == 200
...
def _submit_inbound_doc(auth_headers, inbound_doc_id: int, actual_qty: Decimal) -> None:
    response = client.post(
        ...
        json={"actual_qty": float(actual_qty), "warehouse_id": "CODEX-TEST-WH-M7-001"},
        headers=_warehouse_headers(auth_headers, f"CODEX-TEST-M7-WH-SUB-IN-{uuid4().hex[:6]}"),
    )
    assert response.status_code == 200
...
def _query_receipt_doc(
    *,
    ...
    db = SessionLocal()
    try:
        statement = select(ReceiptDoc).where(ReceiptDoc.contract_id == contract_id, ReceiptDoc.doc_type == doc_type)
...
def _query_payment_doc(
    *,
    ...
    db = SessionLocal()
    try:
        statement = select(PaymentDoc).where(PaymentDoc.contract_id == contract_id, PaymentDoc.doc_type == doc_type)
        ...
        if purchase_order_sales_order_id is None:
            ...
            for doc in docs:
                if doc.purchase_order and doc.purchase_order.source_sales_order_id == purchase_order_sales_order_id:
                    return doc
...
def _recent_30_day_window_utc() -> tuple[datetime, datetime]:
    ...
    end_local = datetime.combine(today_cn + timedelta(days=1), time.min, tzinfo=SHANGHAI_TZ)
    return start_local.astimezone(UTC), end_local.astimezone(UTC)
