...
- 当前自动化基线：已完成首轮自动化回归、稳定性复核与 `D-CHAIN-01`、`D-CHAIN-02`、`D-CHAIN-03`、`D-CHAIN-04` 定向回归，后端全量 `133 passed`、`D-CHAIN-01` 定向后端回归 `25 passed`、`D-CHAIN-02` 定向后端回归 `24 passed`、`D-CHAIN-03` 定向后端回归 `15 passed`、`D-CHAIN-04` 定向后端回归 `10 passed`、管理后台 `pnpm test` 当前为 `76 passed`、`pnpm build` 通过；详见 `docs/history/stage-d/V6阶段D-D01联调执行记录-2026-03-08.md`。
