...
## 1. 文档定位
- 文档状态：`生效`
- 目标：固化旧版阶段 D 的联调范围、环境基线、责任边界、关键链路与回归出口，并作为后续面向生产范围重启阶段 D 的输入基线。
- 使用边界：本清单用于阶段 D 联调与综合回归，不替代 `docs/需求方案.md`、`docs/V6阶段B-详细设计与原型冻结包.md`、`docs/V6生产可用版本治理总纲.md` 与各模块任务包。

## 2. 当前阶段判定
- 当前阶段：`阶段 D - 冻结保留（待 G1 ~ G5 完成后重启）`
- 进入条件：`M1 ~ M7`、`M8-01 ~ M8-28` 已完成并推送，且已完成阶段 C 冻结范围闭合复核、联调范围与真实交付范围一致、降级事项已写入阶段 E 或后续治理池。
- 当前唯一主目标：`无；当前仅保留为后续阶段 D 重启输入`
- 当前结论：旧版阶段 C 冻结范围闭合复核与首轮联调已形成历史基线，但该结论只覆盖旧版交付范围的“可联调”，不足以直接支撑“可真实业务测试、可验收、可上线”判断，当前阶段 D 已冻结保留。
...
- 旧版阶段 D 结果仅代表达到“可联调”级别，不得直接作为当前生产化开发、正式验收或上线判断依据。
...
| 管理后台模式 | `demo/proxy` 双模式保留，旧版阶段 D 以 `proxy` 实联调为主，`demo` 仅用于空态与只读演示兜底；该口径仅属于联调工具，不属于正式交付能力 |
| 小程序模式 | 本地联调与微信登录链路都需回归，优先验证本地联调可稳定切换身份；`dev-login/debug_openid` 仅用于开发/测试调试 |
...
- 旧版阶段 D 通过仅代表达到“可联调”级别；如目标改为“生产可用版本”，必须先回到生产化开发主线并完成 `G1 ~ G5` 后，方可重启新的阶段 D。

## 9. 本轮非目标
- 不新增业务规则、页面能力、接口能力或数据库模型。
- 不在阶段 D 内再次拆分新的 `M8-xx` 功能任务包。
- 不把联调问题处理成无边界的需求扩张；超出当前冻结范围的事项统一进入阶段 E 前的遗留池评估。
- 不把 `demo/proxy/local_api/mock/dev-login/debug_openid` 当作正式交付能力、验收依据或生产入口。
