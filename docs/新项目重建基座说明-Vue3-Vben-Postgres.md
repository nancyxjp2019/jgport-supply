...
- 已完成 `admin-web/` 首批业务页面迁移：`M8-01` 管理后台基座，以及 `M8-16 ~ M8-23` 订单、资金、库存、合同关闭、退款核销、多维报表与页面内导出首批、按钮权限与后端鉴权配套能力；不含 `M8-24` 异步导出编排任务中心。
...
- 当前下一优先方向：`M8-24` 导出编排任务中心首批，其后继续补齐多维钻取与口径重算能力。
