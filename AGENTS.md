...
- 每次进入 Git 提交前，必须先汇报本次实际使用的技能清单、调用顺序与用途；若未使用任何技能，也必须明确说明“本次未使用技能”。
...
- 任一阶段准备切换到下一阶段前，必须先执行一次“冻结范围闭合复核”，以 `docs/需求方案.md`、`docs/V6阶段A-流程图状态机与UI原型清单.md`、`docs/V6阶段B-详细设计与原型冻结包.md`、当前阶段任务包和总控台账作为唯一依据，逐项确认“已完成 / 已降级 / 已合并承接 / 未完成阻断”。
- 未完成“冻结范围闭合复核”前，不得仅因主链可运行、局部测试通过或已有联调草案，就把阶段状态更新为下一阶段执行中。
- 若存在未完成项，必须先判断其是否阻断下一阶段；阻断项必须在当前阶段收口，不阻断项也必须写入明确去向文档后，才能继续推进阶段切换。
- 任何降级、合并承接、延后处理的事项，必须同步更新总控台账、当前阶段收口文档、后续路线图与对应任务包，确保文档口径一致；未完成一致性更新前，视为不得切阶段。
- 阶段 D 启动前，必须同时满足：阶段 C 收口阻断项全部完成并已提交推送、联调范围与真实交付范围一致、待降级事项已进入阶段 E 或后续治理池，不得带着阶段 C 阻断缺口进入 D。
- 若事后发现阶段切换判断错误，必须立即停止下一阶段执行，先回退总控状态并冻结唯一纠偏目标；纠偏目标未完成前，不得并行开启新的阶段性主目标。
...
- 所有以“生产可用版本”为目标的开发、联调、验收、上线判断，必须以 `docs/V6生产可用版本治理总纲.md`、`docs/V6阶段E-验收发布与上线清单.md`、`docs/V6规则-实现-测试-验收追踪矩阵.md` 为当前宏观治理基线。
- 不得将“联调通过、页面可见、局部测试通过、演示模式可运行”判定为“可真实业务测试、可验收或可上线”。
- 涉及生产目标的开发，必须先判断当前事项属于：`G1 前置治理基座`、`G2 正式身份与端收口`、`G3 订单后半状态机闭环`、`G4 真实业务全链路回归`、`G5 发布与上线准备` 中的哪一模块；未归类不得开工。
- 未完成组织、用户、主数据、审批模板、参数配置等前置治理能力前，不得对外宣称已具备真实业务全流程测试或生产可用条件。
- 管理后台正式认证默认沿用 V5 `SUPER_ADMIN + MFA` 基线；小程序正式登录默认沿用 V5 “微信登录 + 绑定/激活”基线。任何替换方案都必须先更新 `docs/需求方案.md` 与宏观治理文档后方可实施。
- `demo`、`proxy`、`local_api`、`mock`、`dev-login`、`debug_openid` 仅属于开发工具能力，不得作为正式交付能力、验收依据或生产入口。
- 任何阶段切换前，必须先执行冻结范围闭合复核；若发现阶段判断错误，必须先冻结唯一纠偏目标，未纠偏完成前不得并行开启新的阶段性主目标。
- 任何降级、遗留、延后事项都必须写入明确承接文档与阶段，不得口头漂移。
