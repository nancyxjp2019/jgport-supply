...
- 文档类开发任务默认使用 `gpt-5.4-high` 执行，适用于需求方案、任务包、README、原型说明、路线图、评审结论等以文档产出为主的工作。
- 代码类开发任务默认使用 `gpt-5.3-codex-high` 执行，适用于后端、小程序、管理后台、测试、脚本、迁移、重构与缺陷修复等以代码改动为主的工作。
- 当前会话内不得声称或假设系统已自动切换模型；若平台不支持会话内切模，只能提示用户手动切换到目标模型。
- 若当前会话绑定模型与任务类型要求不一致，必须先明确提示“需要切换模型”以及建议切换的目标模型；在未切换前，不得声称已自动完成模型切换。
- 若任务同时包含文档与代码，必须先判断主任务类型；若两者都属于核心交付，应拆分为两个阶段，并在阶段切换点明确提示需要切换到对应模型。
- 任何模型切换规则都属于执行流程约束，不改变既有质量门禁、技能调用、自检、独立评审与提交推送要求。
